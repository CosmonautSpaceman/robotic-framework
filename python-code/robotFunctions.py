...
def mdh_calc_transformation(From, to, rb, qc):
    T = np.identity(4)
    From = From +1
    to = to +1
...
def inertiaComp(qc, rb):
    ...
    M = np.zeros((rb.ndof, rb.ndof))
    for i in range(rb.ndof):    
        qdd_in = np.array([qdd[i,:]])
        Q = np.ravel(invdyn(rb, q_in, qd_in, qdd_in, grav))
        M[:,i] = Q
        
    return M


def cgComp(qc,qcd, rb):
    grav = np.array([[0],[0],[-9.81]])    
    qdd = np.zeros((1,rb.ndof))
...
def forwardDynamics(Q, qc, qcdot, rb):
    M = inertiaComp(qc, rb)
    CG = cgComp(qc, qcdot, rb)
...
#     vdot = np.dot(np.transpose(R), grav)
    ...
    #   Forward recursion
        for i in range(N_DOFS):
            T = mdh_calc_transformation(i-1, i, rb, q)
            R = T[:3,:3]
#             print(R)
#             print("\n")
            ...
            wdot_ = (np.dot(R.T, wdot) + 
                     np.dot(z0,qddot[i,k]) + 
                     np.cross(w, np.dot(z0, qdot[i,k]), axis=0))
            
            w_ = (np.dot(R.T,w) + 
                  np.dot(z0, qdot[i,k]))

            print("In-test: vdot:", vdot)
            print("In-test: cross1", np.cross(wdot, p, axis=0))
            print("In-test: cross1",np.cross(w, np.cross(w, p, axis=0), axis=0))
            print("add test: ",(vdot + 
                    np.cross(wdot, p, axis=0) + 
                    np.cross(w, np.cross(w, p, axis=0), axis=0)))
            print(R)
            print(R.T)
            ...
            vcdot = (vdot + np.cross(wdot, rb.joints[i].r, axis=0) + 
                     (np.cross(w, np.cross(w, rb.joints[i].r, axis=0), axis=0)) )
            ...
            print("w:", w.reshape(3))
            print("wdot:", wdot.reshape(3))
            print("vdot:", vdot.reshape(3))
            print("vcdot:", vcdot.reshape(3))
    ...
    #   Backward recursion
        for i in reversed(range(N_DOFS)):
            ...
            if i+1 < N_DOFS:
                ...
                T = mdh_calc_transformation(i, i+1, rb, q)
                R = T[:3, :3]
            else:
                R = np.eye(3)
...
def pathplanning(p0x, p0y, p0z, pfx, pfy, pfz, theta0, phi0, psi0, theta, phi, psi, tf, hz):
    ...
    cartesianTHETA = np.array([theta0, v0, a0, theta, vf, af])
    cartesianPHI = np.array([phi0, v0, a0, phi, vf, af])
    cartesianPSI = np.array([psi0, v0, a0, psi, vf, af])
